module EasyIO
  def execute_out(command, pid_logfile: nil, working_folder: Dir.pwd, regex_error_filters: [], info_on_exception: '', exception_exceptions: [], powershell: false, show_command_on_error: false, raise_on_first_error: true)
    STDOUT.sync = true
    result = nil
    exit_status = nil
    ...
    if powershell
      ...
      mkdir(::File.dirname(ps_script_file))
    ...
    Dir.chdir(working_folder) do
      Open3.popen3(*popen_arguments) do |_stdin, stdout, stderr, wait_thread|
        unless pid_logfile.nil? # Log pid in case job or script dies
          mkdir(::File.dirname(pid_logfile))
  ...
  def powershell_out(ps_script, pid_logfile: nil, working_folder: Dir.pwd, regex_error_filters: [], info_on_exception: '', exception_exceptions: [], show_command_on_error: false)
    execute_out(ps_script, pid_logfile: pid_logfile, working_folder: working_folder, regex_error_filters: regex_error_filters, info_on_exception: info_on_exception, exception_exceptions: exception_exceptions, powershell: true, show_command_on_error: show_command_on_error)
  end
